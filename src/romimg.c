...
static int GetExtInfoStat(const struct ROMImgStat *ImageStat, struct RomDirFileFd* fd, unsigned char type, void** buffer, unsigned int nbytes){
	...
	while(offset<fd->ExtInfoEntrySize){
		ExtInfoEntry=(struct ExtInfoFieldEntry*)((unsigned int)ImageStat->image+fd->ExtInfoOffset);
		...
		if(ExtInfoEntry->type==EXTINFO_FIELD_TYPE_DATE || ExtInfoEntry->type==EXTINFO_FIELD_TYPE_COMMENT){
			...
			printf("DEBUG: Unknown EXTINFO entry type: 0x%02x @ ROM offset: 0x%x, EXTINFO offset: %u\n", ExtInfoEntry->type, fd->ExtInfoOffset+offset, offset);
			result=EIO;
			break;
		}
	}
...
int CreateBlankROMImg(const char* filename, ROMIMG* ROMImg){
	...
	char UserName[32], LocalhostName[32], cwd[128];
	...
	sprintf(ROMImg->comment, "%08x,conffile,%s,%s@%s/%s", ROMImg->date, filename, UserName, LocalhostName, cwd);
	...
	memcpy((void*)((unsigned int)ResetFile->ExtInfoData+sizeof(struct ExtInfoFieldEntry)), &ROMImg->date, ExtInfoEntry->ExtLength);
...
int WriteROMImg(const char* file, const ROMIMG* ROMImg){
	...
	for(i=0,TotalExtInfoSize=0; i<ROMImg->NumFiles; i++){
			...
			if(ROMImg->files[i].RomDir.ExtInfoEntrySize%4!=0){
				printf("ASSERT ROMImg->files[%d].RomDir.ExtInfoEntrySize%4==0\n", i);
				abort();
			}
	}
	...
	if((extinfo=malloc(TotalExtInfoSize))!=NULL){
		...
		if((OutputFile=fopen(file, "wb"))!=NULL){
			...
			if((FileAlignMargin=(ftell(OutputFile)%16))>0) fseek(OutputFile, 16-FileAlignMargin, SEEK_CUR);
	...
	if((InputFile=fopen(path, "rb"))!=NULL){
		...
		if((ImageStat.image=malloc(ImageStat.size))!=NULL){
			if(fread(ImageStat.image, 1, ImageStat.size, InputFile)==ImageStat.size){
				...
				if(result==0){
					ImageStat.ROMFS_start=(void*)((unsigned int)ImageStat.image+DataStartOffset);
					if(OpenRomDirFile(&ImageStat, "RESET", &RomDirFileFd)!=0){
						printf("DEBUG: Unable to locate RESET!\n");
						free(ImageStat.image);
						fclose(InputFile);
						return -1;
					}
					if(RomDirFileFd.size!=DataStartOffset){
						printf("DEBUG: Size of RESET does not match the start of ROMFS!\n");
						free(ImageStat.image);
						fclose(InputFile);
						return -1;
					}
					...
					if(OpenRomDirFile(&ImageStat, "ROMDIR", &RomDirFileFd)!=0){
						printf("DEBUG: Unable to locate ROMDIR!\n");
						free(ImageStat.image);
						fclose(InputFile);
						return -1;
					}
					...
					while(RomDir->name[0]!='\0'){
						if(strncmp(RomDir->name, "ROMDIR", sizeof(RomDir->name))!=0 && strncmp(RomDir->name, "EXTINFO", sizeof(RomDir->name))!=0){
							...
							memcpy(file->ExtInfoData, (void*)((unsigned int)ImageStat.image+RomDirFileFd.ExtInfoOffset+ExtInfoOffset), RomDir->ExtInfoEntrySize);
							file->FileData=malloc(RomDir->size);
							memcpy(file->FileData, (void*)((unsigned int)ImageStat.image+offset), RomDir->size);
						}
...
void UnloadROMImg(ROMIMG* ROMImg){
	...
	if(ROMImg->files!=NULL){
		for(i=0; i<ROMImg->NumFiles; i++){
			if(ROMImg->files[i].ExtInfoData!=NULL) free(ROMImg->files[i].ExtInfoData);
			if(ROMImg->files[i].FileData!=NULL) free(ROMImg->files[i].FileData);
		}
	...
	if(strcmp("RESET", filename)){
		for(result=ENOENT,i=0,file=ROMImg->files; i<ROMImg->NumFiles; i++,file++){
			if(strncmp(file->RomDir.name, filename, sizeof(file->RomDir.name))==0){
				if(file->FileData!=NULL) free(file->FileData);

				if(file->ExtInfoData!=NULL) free(file->ExtInfoData);
				for(; i<ROMImg->NumFiles; i++) memcpy(&ROMImg->files[i], &ROMImg->files[i+1], sizeof(struct FileEntry));
